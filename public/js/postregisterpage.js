(function($) {
  ...
  tlScreen1
    ...
    .from(
      "#top-bubbles",
      4,
      { opacity: 0, y: 50, x: 40, ease: Elastic.easeOut.config(1, 0.8) },
      "start"
    )
    .to(
      "#btnClick",
      3.5,
      { opacity: 1, y: -96, ease: Elastic.easeOut.config(1, 1) },
      "-=3.5"
    );
  ...
  $("#invert").click(function() {
    if (color == green) {
      ...
      $(".screen p").html(
        "Your file has not been transferred successfully via bluetooth."
      );
      color = red;
      tlScreen1.restart();
    } else {
      ...
      $(".screen p").html(
        "You have successfully transferred your file via bluetooth."
      );
      color = green;
      tlScreen1.restart();
    }
